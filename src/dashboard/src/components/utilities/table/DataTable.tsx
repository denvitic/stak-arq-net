    ...
    const baseColumns = keys.map((col) => ({
      ...
      cell: (info: CellContext<T, unknown>) => {
        ...
        if (col.toLowerCase().includes('id')) {
          return (
            <span className="text-gray-900 font-medium max-w-50 truncate whitespace-nowrap">
              {renderValue(value)}
            </span>
          );
        }
                ...
                {keys.map((k) => {
                  ...
                  return (
                    <span
                      ...
                      className={
                        k === 'name'
                          ? 'text-gray-900 font-semibold max-w-50 truncate whitespace-nowrap pe-6'
                          : 'text-sm text-gray-500 max-w-50 truncate whitespace-nowrap pe-6'
                      }
                    >
        ...
        if (
          ...
          return (
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-900 font-semibold max-w-50 truncate whitespace-nowrap">
                {renderValue(value)}
              </span>
            </div>
          );
        ...
        return (
          <span className="text-gray-900 font-medium max-w-50 truncate block ">
            {renderValue(value)}
          </span>
        );
      },
          ...
          <>
            ...
            <div className="p-4 pt-0 flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <Input
                  type="text"
                  className="max-w-96 lg:min-w-96 min-w-full placeholder:text-gray-400 bg-white text-gray-900 border-gray-300"
                  value={globalFilter ?? ''}
                  onChange={(e) => setGlobalFilter(e.target.value)}
                  placeholder="Search your relevant items..."
                />
                <Button onClick={handleDownload} className="p-2 px-4 rounded-md bg-gray-900 text-white hover:bg-black">
            ...
            {/* Table */}
            <div className="overflow-x-auto border rounded-lg border-gray-200 bg-white">
              <Table>
                <TableHeader>
                  {table.getHeaderGroups().map((headerGroup) => (
                    <TableRow key={headerGroup.id} className="border-b border-gray-200 bg-gray-50/75">
                      {headerGroup.headers.map((header) => (
                        <TableHead
                          key={header.id}
                          className="cursor-pointer select-none min-w-42 px-0 text-gray-700 font-semibold"
                        >
                          {header.isPlaceholder ? null : (
                            <Button
                              className="flex items-center gap-1 px-4 bg-transparent hover:bg-transparent text-gray-800 font-semibold"
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-gray-200">
              ...
              <div className="flex items-center gap-2">
                <Label
                  htmlFor="pageSize"
                  className="mr-0 text-gray-700 text-sm font-medium whitespace-nowrap min-w-32"
                >
                  Rows per page:
                </Label>
                <Select
